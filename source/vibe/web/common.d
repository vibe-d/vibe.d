...
// Leave it undocummented: It's not yet implemented in the REST and web interface.
/*
 ...
 *
 * ----
 * @fromBody("pack", "package")
 * void ship(int pack);
 * // The server will receive the following body for a call to ship(42):
 * // { "package": 42 }
 * ----
 */
WebParamAttribute fromBody(string identifier, string field) {
	if (!__ctfe)
		assert(false);
 ...
 * @fromHeader("auth", "Authorization")
 * void login(string auth);
 * ----
 */
WebParamAttribute fromHeader(string identifier, string field) {
	if (!__ctfe)
		assert(false);
 ...
 * ----
 */
WebParamAttribute fromQuery(string identifier, string field) {
